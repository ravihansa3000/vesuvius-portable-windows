...
# *****************************************************************
# Uniform Server MPG - 13-8-2010
# Original script from http://pear.php.net/go-pear.
# Modifications:
# 1) Changed forward slash for backward slash in these lines: 
# line 202 : 'php_dir'   => '$prefix\PEAR',
# line 206 : 'temp_dir'   => '$prefix\temp',
# line 207 : 'cache_dir' => '$php_dir\cache',
# line 209 : 'webfrontend_file' => '$prefix\index.php',
#
# 2) Added new block starting at line 2253
#    Corrects malformed link and switches in new index.php page
#    Deletes index.php and copies uniform_server_index.php
#    to index.php -- switches in new file
# 3) Updated for   GO_PEAR_VER', '1.1.2'
# Uniform Server MPG - 25-1-2011
# Modifications:
# 1) Removed background gifs prevents errors reported in log
# 2) Replaced this setion: 
...
#    Now selects new Pear version
#    Note: Still reports deprecated errors for PHP 3.5 
# 4) Updated for   GO_PEAR_VER', '1.1.6'
# Uniform Server MPG - 18-6-2011
# 
# *****************************************************************
...
function download_url($url, $destfile = null, $proxy = null)
{
    ...
    while ($line = fgets($fp, 2048)) {
        ...
        if ($use_suggested_filename && !strncasecmp($line, $cdh, $cdhl)) {
            if (preg_match('/filename="([^"]+)"/i', $line, $matches)) {
...
<p>
<b>Note:</b> <i>Uniform Server pre-configures user settings hence there is no need to change any values.</i> 
</p>
...
?>
	    ...
	    <p>
            Below is a suggested file layout for your new PEAR installation<br /><b><i>Note</i></b>: No need to change for Uniform Server.
        </p>
...
?>
            ...
            <br/>
            <b>Note:</b><i> Above path already pre-configured in Uniform Server hence you are ready to go.</i><br/><br/>
...
<?php
//######################################################################
// MPG Fix malformed link and switch in Uniform Server specific index.php
// front-end page.
 if ( WINDOWS ) {
  $next = str_replace('\\', '', strtolower($next)); // Remove back slash
  $path = realpath('../');    // currently in temp, move up get real path 
  unlink("$path\index.php");  // delete old index
  copy  ("$path\uniform_server_index.php","$path\index.php"); // replace  
}
//########################################################################
?>
