...
#        Added support for portable Firefox and PAC
#############################################################################
*/
...
if(@$_POST["host_navn"]) {
  ...
  //-- If portable Firefox write to PAC file otherwise write to PC hosts file
  $port = get_apache_port();  // Get Apache port   
  ...
  else{ // Portable Firefox not being used
  // Write to hosts file
...
//===
  ...
  if(is_file($pc_hosts)) {    // findes filen hosts?
   ...
   if($ialt > 0) {
      for($c = 0; $c < $ialt; $c++) {
         echo"<a target=\"_new\" href=\"http://". $host_name[$c] .":$port/\">". $host_name[$c] ."</a><br />";
      }
      echo"<br />";
   }
