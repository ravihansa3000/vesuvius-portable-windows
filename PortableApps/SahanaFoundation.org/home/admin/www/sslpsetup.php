...
if ( !(file_exists(USF_SSL_PASSWORD)) ) {        // Does file exist
	$FHandle = fopen(USF_SSL_PASSWORD, 'w'); // no: create it
	fclose($FHandle);
}
...
if (@$_POST['submit']) {
	...
	$wfile = fopen(USF_SSL_PASSWORD, 'w') ;
	fwrite($wfile, $urdata);
	fclose($wfile);
?>
...
<?php
	...
	$tfile = fopen(USF_SSL_PASSWORD, "r");
