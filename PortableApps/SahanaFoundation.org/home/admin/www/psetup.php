...
if ( !(file_exists(USF_WWW_PASSWORD)) ) {         // Does file exist
	$FHandle = fopen(USF_WWW_PASSWORD, 'w');  // no: create it
	fclose($FHandle);
}
...
if (@$_POST['submit']) {
	...
	$wfile = fopen(USF_WWW_PASSWORD, 'w') ;
	fwrite($wfile, $urdata);
	fclose($wfile);
?>
...
<?php
	...
	$tfile = fopen(USF_WWW_PASSWORD, "r");
