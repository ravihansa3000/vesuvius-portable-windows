...
# 16-2-2010 - Updated to support PAC file 
#  5-2-2011 - Updated vhost in Apache config replaced * with *:80 
#############################################################################
*/
...
@$apache = new Config (USF_APACHE_CNF,"#");
...
// Update PAC file if using portable Firefox
if(file_exists(USF_HOSTS_PAC)){ // Does PAC file exists 
  $str=file_get_contents(USF_HOSTS_PAC);              // read entire file
  $str1=preg_replace("/:[\d]+/",":".$portnum, $str ); // Search replace
  file_put_contents($file,$str1);                     // rewrite entire file

  $str=file_get_contents(USF_APACHE_CNF);              //read entire file
  $str1=preg_replace("/localhost:[\d]+/","localhost:".$portnum, $str );
  $test=file_put_contents($file,$str1);                //rewrite entire file
 }//end udate PAC file

// Update Vhost ports
  $str=file_get_contents(USF_APACHE_CNF);              //read entire file
  $str1=preg_replace("/:[\d]+/",":".$portnum, $str );
  $test=file_put_contents(USF_APACHE_CNF,$str1);       //rewrite entire file
